...
"""
    ...
    if 'PROMPT_PREFIX' in os.environ:
        prompt_prefix = os.environ['PROMPT_PREFIX']
    else:
        prompt_prefix = "User: "

    if 'RESULT_PREFIX' in os.environ:
    ...
    while True:
        ...
        if prompt[-1] == "🤖":
            ctx = ctx + prompt_prefix + prompt[:-1]
        else:
            ctx = ctx + prompt_prefix + prompt + "\n" + result_prefix

        prompt = prompt + result_prefix
